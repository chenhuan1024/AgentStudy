...
- 选站页将“ID 匹配配置”（话务 ID / 工参 ID / 选站小区字段）合并到预览区顶部，放在“基站名字段”之前；“预览选站话务”按钮单独置于下一行并高亮显示。
- 选站页“站去重数 / 小区去重数”统一基于“上传选站文件/粘贴选站数据”的原始选站表统计，不依赖“选站数据分析”预览结果。
- 点击“刷新图表”时，条件组图改为饼图（命中条件 vs 其他）；占比按“命中数量 / 筛选前总量”计算；同字段多条件会自动合并成一张饼图（例如 `1<=A<=2`）。
- 新增“选站配置导出”分页：支持粘贴/上传基站名单，并上传配置压缩包或“包含压缩包/配置文件的文件夹”（支持 `zip/rar/tar.gz/tgz/tbz2/txz` 等常见多层嵌套压缩）；按“基站名 = 配置目录名/文件名（完全匹配）”筛选并导出到 `exports` 目录；返回导出文件数和命中基站数。
...
- `/condition_chart_counts`：按条件组统计筛选前后数量，用于条件组饼图（含同字段条件合并）。
- `/station_distinct_count`：按原始选站表统计字段去重数（站数/小区数）。
  - `/station_config_export`：基站名单 + 配置压缩包（或压缩包文件夹）联合筛选并导出匹配配置目录（递归解包，多层压缩可处理）。
...
- **主流程**
  ...
  5. 配置条件组并执行关联筛选；
  6. 按组预览与导出结果。
  7. （新增）在“选站配置导出”分页上传名单与配置包，筛选并导出匹配配置目录。
- **界面结构**
- 左侧：上传、条件组配置与执行、导出入口。
  - 右侧：四页签（工参/话务/选站/选站配置导出），其中前三页为“预览 + 分布图”双区域工作区。
  - 每个分析页支持拖拽分割上下区域高度。
